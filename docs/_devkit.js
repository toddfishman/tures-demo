/* ============================================================
   TURES — Explore Dev Toolkit  (experiment-only, not shipped)
   Injected into every /explore mockup. Gives a floating panel to:
...
(function () {
  ...
  function applyPalette(p) {
    ...
    // backgrounds (all candidate names across the four directions)
  ...
  /* ---------- init ---------- */
  var IN_IFRAME = false; try { IN_IFRAME = window.top !== window.self; } catch (e) { IN_IFRAME = true; }
  function parseHashPal() { // deep-link a palette via  #pal=BG-ACC-TX  (hex, no #)
    var m = (location.hash || "").match(/pal=([0-9a-fA-F]{3,6})[,\-]([0-9a-fA-F]{3,6})[,\-]([0-9a-fA-F]{3,6})/);
    return m ? { bg: "#" + m[1], acc: "#" + m[2], tx: "#" + m[3] } : null;
  }
  function revealAll() { // force-show staged/animated content (for static side-by-side thumbnails)
    Array.prototype.forEach.call(document.querySelectorAll("body *"), function (el) {
      if (el.closest("#dvk")) return;
      if (getComputedStyle(el).opacity === "0") { el.style.setProperty("opacity", "1", "important"); el.style.setProperty("transform", "none", "important"); }
    });
  }
  function start() {
    var hp = parseHashPal();
    if (IN_IFRAME) { // embedded thumbnail: no panel, paint the deep-linked palette, freeze + reveal
      var ks = document.createElement("style"); ks.textContent = "*{animation:none !important;transition:none !important;}"; document.head.appendChild(ks);
      if (hp) applyPalette(hp);
      revealAll(); setTimeout(revealAll, 300); setTimeout(revealAll, 1000);
      return;
    }
    buildPanel();
    if (hp) {
      applyPalette(hp);
      var b = document.getElementById("dvk-bg"), a = document.getElementById("dvk-acc"), t = document.getElementById("dvk-tx");
      if (b) b.value = hp.bg; if (a) a.value = hp.acc; if (t) t.value = hp.tx;
    } else if (state.palette) applyPalette(state.palette);
    renderImages();
  }
