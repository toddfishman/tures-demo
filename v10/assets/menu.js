...
(function () {
  ...
  // Greenhouse: the dark-botanical direction, applied on EVERY page. Replaces the retired
  // parchment/oxblood themes and v8's dark-mode toggle. Theme tokens live in tures.css under
  // html[data-theme="greenhouse"].
  document.documentElement.setAttribute('data-theme', 'greenhouse');

  // Greenhouse scene accent: swaps the accent tokens site-wide (Rainforest/Arctic/Savanna/Beach/
  // New York). Replaces the old 4-season picker. Persisted to localStorage 'tures.scene'.
  var ACCENT_KEY = 'tures.scene';
  var SCENE_IDS = ['rainforest', 'arctic', 'savanna', 'beach', 'city'];
  var accent = 'rainforest';
  try { var _a = localStorage.getItem(ACCENT_KEY); if (SCENE_IDS.indexOf(_a) > -1) accent = _a; } catch (e) {}
  document.documentElement.setAttribute('data-scene', accent);
  ...
  // Pages still wearing the legacy rotating destination backdrop. Rebuilt Greenhouse
  // pages bring their own hero imagery, so drop each from this map as it's rebuilt.
  var SCENE_PAGES = {
    '02-taste-engine.html': 1, '04-connections.html': 1,
  ...
  function init() {
    ...
    // floating scene-accent picker, lower-left — the Greenhouse 5-scene package
    var sw = el('div', 'tures-theme-switch');
    sw.setAttribute('role', 'group'); sw.setAttribute('aria-label', 'Scene accent');
    sw.appendChild(el('span', 'ts-label', 'Scene'));
    var SCENES = [['rainforest', 'Rainforest', '#4ADE80'], ['arctic', 'Arctic', '#7DD3FC'], ['savanna', 'Savanna', '#F59E0B'], ['beach', 'Beach', '#2DD4BF'], ['city', 'New York', '#FFE55C']];
