...
const SYSTEM = loadPlaybook("plan");
...
export async function runAgentLoop(tripId: string, brief: Brief, accountId = "demo", context?: TravelerContext): Promise<ProposedPlan> {
  ...
  const messages: any[] = [
    {
      role: "user",
      content:
        (context?.prose ? `TRAVELER CONTEXT:\n${context.prose}\n\n` : "") +
        `BRIEF:\n${JSON.stringify(brief, null, 2)}\n\n` +
        `Find and propose the best flight + stay for this traveler, honoring their budget posture.`,
    },
  ];
