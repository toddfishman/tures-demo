...
5. **Self-host images** — replace hotlinked `images.unsplash.com/...` with optimized, locally-served responsive images (licensing + speed + reliability).
6. **Add share/SEO meta** — `<meta name="description">`, OpenGraph + Twitter-card tags, and a favicon on every `v8` page. The pitch is "share your trip," so link previews matter.
7. **Bigger, needs Todd's input first** (don't just do these): execution-dashboard redesign and the palette pick — see the bullets above; design calls, not pure cleanup.
